...
import { Key, AppToDeskThingData, AppManifest, AppDataInterface, AppSettings, DESKTHING_EVENTS, Action, EventMode, Task, Step, SavedData, SongEvent, SettingOption, SongData, GenericTransitData, TransitData, DeskThingToAppCore } from "@deskthing/types";
type ImageReference = {
    [key: string]: string;
};
type BaseData = TransitData<string, string, unknown>;
type ExtractDeskThingType<D extends BaseData> = D["type"] | DESKTHING_EVENTS | SongEvent;
type ExtractDeskThingRequest<D extends BaseData, T extends ExtractDeskThingType<BaseData>> = Extract<DeskThingToAppCore, {
    type: T;
}>["request"] | Extract<D, {
    type: T;
}>["request"];
type ExtractDeskThingPayload<B extends BaseData, T extends ExtractDeskThingType<B>, R extends ExtractDeskThingRequest<B, T> | undefined> = Extract<DeskThingToAppCore, {
    type: T;
    request?: R;
}> extends never ? Extract<B, {
    type: T;
    request: R;
}> : Extract<DeskThingToAppCore, {
    type?: T;
    request?: R;
}>;
type DeskThingListener<D extends BaseData, T extends ExtractDeskThingType<D>, R extends ExtractDeskThingRequest<D, T>, P extends undefined | never = never> = (payload: ExtractDeskThingPayload<D, T, R> | P) => void | Promise<void>;
/**
 ...
 */
export declare class DeskThingClass<ToAppData extends BaseData = GenericTransitData, ToClientData extends BaseData = GenericTransitData> {
     ...
     * });
     */
    static getInstance<ToAppData extends BaseData = GenericTransitData, ToClientData extends BaseData = GenericTransitData>(): DeskThingClass<ToAppData, ToClientData>;
    /**
     ...
     */
    on<E extends ExtractDeskThingType<ToAppData>>(event: E, callback: DeskThingListener<ToAppData, E, string>): () => void;
    /**
     ...
     */
    off<E extends ExtractDeskThingType<ToAppData>>(event: E, callback: DeskThingListener<ToAppData, E, string>): void;
    /**
     ...
     */
    once<T extends ExtractDeskThingType<ToAppData>, R extends ExtractDeskThingRequest<ToAppData, T>>(event: T, callback?: DeskThingListener<ToAppData, T, R>, request?: R): Promise<ExtractDeskThingPayload<ToAppData, T, R>>;
    /**
     ...
     * @param callback
     */
    fetch: <T extends ExtractDeskThingType<ToAppData>, R extends ExtractDeskThingRequest<ToAppData, T>>(requestData: AppToDeskThingData | ToClientData, listenData?: {
        type: T;
        request?: R;
    }, callback?: (data: ExtractDeskThingPayload<ToAppData, T, R> | undefined) => void, timeoutMs?: number) => Promise<ExtractDeskThingPayload<ToAppData, T, R> | undefined>;
    /**
     ...
     * });
     */
    send(payload: ToClientData & {
        app?: string;
        clientId?: string;
    }): void;
    sendSong(songData: SongData): void;
    /**
     ...
     */
    getData(): Promise<SavedData | null>;
    /**
     * Fetches data from the server if not already retrieved, otherwise returns the cached data.
     * This method also handles queuing requests while data is being fetched.
     *
     ...
     * console.log('Fetched data:', data);
     */
    getAppData(): Promise<AppDataInterface | null>;
    /**
     ...
     */
    setSettingOptions: (settingId: string, options: SettingOption[]) => Promise<void>;
    /**
    ...
    tasks: {
         ...
         * });
         */
        add: (taskData: Omit<Task, "source">) => void;
        /**
         * Initializes the tasks
         * @throws {Error} - when the data is invalid.
         */
        initTasks: (taskData: Record<string, Omit<Task, "source">>) => Promise<void>;
        /**
         ...
         * });
         */
        update: (taskId: string, task: Partial<Task>) => void;
        /**
    ...
    steps: {
         ...
         */
        add: (taskId: string, stepData: Step) => void;
        /**
         ...
         */
        update: (taskId: string, stepId: string, updates: Partial<Step>) => void;
        /**
         ...
         */
        complete: (taskId: string, stepId: string) => void;
        /**
         ...
         */
        restart: (taskId: string, stepId: string) => void;
        /**
         ...
         */
        get: (taskId: string, stepId: string) => void;
    };
    /**
     ...
     */
    saveAppData(data: AppDataInterface, sync?: boolean): void;
    /**
     ...
     * @param data - The data to be saved and merged with existing data.
     */
    saveData(data: SavedData, sync?: boolean): void;
    /**
     ...
     * }, 1000);
     */
    setInterval(task: () => Promise<boolean | void>, timeout?: number): () => void;
    /**
     ...
     */
    addThread(workerPath: string, workerData?: any): [worker: Worker, terminate: () => void];
    /**
     * -------------------------------------------------------
 ...
 */
export declare function createDeskThing<ToAppData extends BaseData = GenericTransitData, ToClientData extends BaseData = GenericTransitData>(): DeskThingClass<ToAppData, ToClientData>;
/**
 ...
 */
export declare const DeskThing: DeskThingClass<GenericTransitData, GenericTransitData>;
export {};
//# sourceMappingURL=deskthing.d.ts.map